...
gs4_auth()
ss <- "https://docs.google.com/spreadsheets/d/1fVP5npbMgBwUumZBi67hiGN6CAAOLrn-2QaTWm1VQDw/edit?usp=sharing"
dat <- read_sheet(ss)
Sys.Date()
newdata <- data.frame(Sys.Date(),("howdy"))
ss %>% 
  sheet_append(newdata)

dat2 <- read_sheet(ss) %>% 
  mutate("Date"=as.Date.(1614988800))
...
## barriers and comments to uppercase
